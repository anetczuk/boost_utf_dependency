...
#ifndef UT_PRIVATE_ACCESS_FIELD_H_
#define UT_PRIVATE_ACCESS_FIELD_H_
...
template<class Tag>
struct stowed {
	static typename Tag::type accessor;
};
template<class Tag>
typename Tag::type stowed<Tag>::accessor;
...
//	std::cout << a.*stowed<A_x>::accessor << std::endl;
//}
//;
...
#define REGISTER_STOW(invokerName, className, field)			\
		template class stow_private<invokerName, &className::field> ;
...
#define REGISTER_FIELD_INVOKER_NAMED(invokerName, className, field, returnType)			\
		...
		REGISTER_STOW(invokerName, className, field)
...
#define CALL_FIELD_INVOKER(invokerId, object)							\
		(object.* stowed<invokerId>::accessor)

#endif /* UT_PRIVATE_ACCESS_FIELD_H_ */
